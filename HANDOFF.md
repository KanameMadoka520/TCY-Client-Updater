...
7. [已经做完了什么](#7-已经做完了什么)
8. [v1.0.7 收尾、验收与后续优化](#8-v106-收尾验收与后续优化如果你要继续)
...
不过从 v1.0.7 后期开始，一些新功能已经逐步拆成独立模块，例如 `jvm_advisor.py` 和 `system_overview.py`。前端仍然统一通过 `Api` 调用它们，主程序负责桥接、目录路径和共享状态。
...
| [1.0.7版本规划方向.txt](1.0.7版本规划方向.txt) | v1.0.7 各 Phase 的原始讨论和设计思路 | 了解为什么要做这些功能 |
