...
**注意事项**：
- Modrinth API v2 公开免费，无需 API Key，但必须设置 `User-Agent` 头（当前使用 `TCYClientUpdater/1.0.6 (tcymc.space)`）。
...
这是项目的历史设计决策。因为 pywebview 加载单文件最简单，且打包时只需把一个文件打入 EXE。虽然文件很大，但通过搜索函数名可以快速定位。注意：从 v1.0.6 开始，NBT 编辑器的前端代码已分离到独立的 `TCYNBTeditor.html`，因为它运行在独立窗口中，与主程序前端没有 CSS/JS 依赖关系。
